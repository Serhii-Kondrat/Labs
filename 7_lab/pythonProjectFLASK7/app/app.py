...
@app.route('/logout2')
def logout2():
    session.pop('username', None)  # Очищення сесії після виходу
    flash('Ви вийшли успішно', 'success')
...
def new_login():
    if request.method == 'POST':
        ...
        # Отримання користувача з бази даних RegUser
        user = RegUser.query.filter_by(RegName=username).first()

        if user and check_password_hash(user.RegPassword, password):
...
def register():
    if request.method == 'POST':
        ...
        if name and password:
            hashed_password = generate_password_hash(password)  # Генеруємо хеш паролю
            new_reg_user = RegUser(RegName=name, RegPassword=hashed_password)
