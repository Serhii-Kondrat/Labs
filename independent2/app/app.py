...
@app.route('/independent', methods=['GET', 'POST'])
@login_required
def independent():
    ...
    return render_template("independent.html", posts=all_independent, access_token=access_token, user_id=user_id, username=username)
...
def new_login():
    if request.method == 'POST':
        ...
        if user and user.RegPassword == password:
            ...
            # Зберігаємо токен користувача у сесії
            session['username'] = username
            session['email'] = user.email
            # Створюємо та передаємо токен користувача на сторінку independent
